...
const EXPERT_PERSONA = `You are a City Manager with significant AI/LLM skills. You are tasked with summarizing the key points from a meeting and responding in a structured manner.`;
const CONFUSED_PERSONA = `You are a local resident overwhelmed by the task of summarizing the key points from a meeting. The good news is that you are comfortable with JSON format.`;
